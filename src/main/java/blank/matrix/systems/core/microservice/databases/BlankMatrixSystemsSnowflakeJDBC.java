package blank.matrix.systems.core.microservice.databases;
...
@Component
public class BlankMatrixSystemsSnowflakeJDBC {
    ...
    //default constructor
    public BlankMatrixSystemsSnowflakeJDBC() {
    }
    ...
    //entry main method
    public DataWrapper getData(String query, String numberOfResults) throws Exception {
        DataWrapper wrapped = new DataWrapper();
        String executableQuery = "";
        if(StringUtils.isNotBlank(query)) {
            if(!StringUtils.containsIgnoreCase(query, "LIMIT")) {
                if(query.contains(";")) {
                    executableQuery = query.replace(";", "").concat(" LIMIT ").
                            concat(StringUtils.isEmpty(numberOfResults) ? "50000" : numberOfResults).concat(";");
                } else {
                    executableQuery = query.concat(" LIMIT ").concat(StringUtils.isEmpty(numberOfResults) ? "50000" : numberOfResults).concat(";");
                }
            } else {
